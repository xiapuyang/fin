...
if [ -f "$PID_FILE" ]; then
    ...
    if kill -0 "$PID" 2>/dev/null; then
        echo "fin server is already running (PID: $PID). Stop it first."
        exit 1
    else
...
if [ -n "$OCCUPANT" ]; then
    echo "ERROR: Port $PORT is already in use by PID $OCCUPANT. Run ./stop.sh first."
    exit 1
fi
...
echo "Waiting for fin server to bind port $PORT..."
SUCCESS=0
i=1
while [ $i -le 30 ]; do
...
if [ "$SUCCESS" = "1" ]; then
    echo "fin server started (PID: $(cat "$PID_FILE")). Listening on http://localhost:$PORT"
    exit 0
else
    if lsof -Pi :$PORT -sTCP:LISTEN -t >/dev/null 2>&1; then
        echo "ERROR: Port $PORT is occupied by another process."
    else
        echo "ERROR: fin server failed to bind port $PORT within 15s. Check logs/fin.log."
    fi
    exit 1
fi
