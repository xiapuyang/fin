...
class TestNoMigrationNeeded:
    def test_skips_when_new_data_dir_already_has_db(self, tmp_path):
    ...
    def test_skips_when_no_legacy_db_exists(self, tmp_path):
    ...
    def test_skips_when_legacy_dir_equals_data_dir(self, tmp_path):
...
class TestMigrationFromHomeFin:
    def test_copies_files_to_new_data_dir(self, tmp_path):
        home_fin = tmp_path / ".fin"
    ...
    def test_legacy_dir_is_preserved(self, tmp_path):
        home_fin = tmp_path / ".fin"
        _make_data_dir(home_fin / "data")
        new_dir = tmp_path / "AppData" / "Fin" / "Fin"
        new_dir.mkdir(parents=True)
    ...
    def test_file_contents_are_preserved(self, tmp_path):
        home_fin = tmp_path / ".fin"
    ...
    def test_creates_new_data_dir_if_missing(self, tmp_path):
        home_fin = tmp_path / ".fin"
        _make_data_dir(home_fin / "data")
        new_dir = tmp_path / "AppData" / "Fin" / "Fin"
...
class TestMigrationFromProjectRoot:
    def test_copies_files_from_project_data_dir(self, tmp_path):
    ...
    def test_home_fin_takes_priority_over_project_root(self, tmp_path):
        home_fin = tmp_path / ".fin"
...
class TestIdempotency:
    def test_does_not_migrate_twice(self, tmp_path):
        home_fin = tmp_path / ".fin"
        _make_data_dir(home_fin / "data")
        new_dir = tmp_path / "AppData" / "Fin" / "Fin"
        new_dir.mkdir(parents=True)
