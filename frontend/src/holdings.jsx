...
const DividendCalendar = ({ incomeItems, positions = [], acctCcy = "CNY", acctFx = 1 }) => {
  ...
  const eventsByDate = React.useMemo(() => {
    const m = {};
    const add = (date, event) => { if (!m[date]) m[date] = []; m[date].push(event); };
  ...
  const upcoming = React.useMemo(() => {
    return Object.entries(divData)
      ...
      .map(([code, d]) => {
        ...
        const freq = Math.max(hist.filter(h => new Date(h.date) >= oneYearAgo).length, 1);
        const perPayment = d.annual_rate ? d.annual_rate / freq : null;
        return { code, ex_date: d.ex_date, annual_rate: d.annual_rate, per_payment: perPayment };
      })
  ...
  return (
    <div>
      {/* Annual dividend estimate summary */}
      {totalEstAnnual > 0 && (
          ...
          <Card padding={20}>
              ...
              {cells.map((day, idx) => {
                ...
                return (
                  <div key={day} onClick={() => hasAny ? setSelectedDay(day === selectedDay ? null : day) : null}
                    ...
                    <div style={{ fontSize: 12, fontWeight: isToday ? 700 : 400, color: isSel ? "white" : isFuture ? "var(--ink-4)" : "var(--ink-3)" }}>{day}</div>
                    {hasAny && (
            ...
            <div style={{ display: "flex", gap: 14, marginTop: 14, borderTop: "1px solid var(--line)", paddingTop: 10 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 5, fontSize: 11, color: "var(--ink-4)" }}>
                <span style={{ width: 8, height: 8, borderRadius: "50%", background: "var(--up)", display: "inline-block" }}/>已录入收入
              </div>
              <div style={{ display: "flex", alignItems: "center", gap: 5, fontSize: 11, color: "var(--ink-4)" }}>
                <span style={{ width: 8, height: 8, borderRadius: "50%", background: "rgba(31,138,76,0.4)", display: "inline-block" }}/>历史除权日
              </div>
              <div style={{ display: "flex", alignItems: "center", gap: 5, fontSize: 11, color: "var(--ink-4)" }}>
                <span style={{ width: 10, height: 10, borderRadius: 2, border: "1.5px dashed var(--up)", display: "inline-block" }}/>即将除权
              </div>
            </div>
